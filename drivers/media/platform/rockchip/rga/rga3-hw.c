...
{
	return fmt->hw_format <= RGA3_COLOR_FMT_LAST_OUTPUT && !fmt->input_only;
}
...
static void rga3_cmd_set_win0_format(struct rga_ctx *ctx)
{
	...
	unsigned int act_xoff, act_yoff;
	bool r2y, y2r;
	u8 rd_format;
	...
	/*
	 * Per TRM Fig. 5-3 "RGA3 Size": SRC_SIZE is the full/virtual source
	 * buffer (matches VIR_STRIDE); ACT_OFF+ACT_SIZE is the actual/active
	 * crop window read from within it and fed to the scaler. This also
	 * supports basic input cropping for 1088px-aligned inputs.
	 */
	src_h = ctx->in.pix.height;
	src_w = ctx->in.pix.width;
	act_h = ctx->in.crop.height;
	act_w = ctx->in.crop.width;
	act_yoff = ctx->in.crop.top;
	act_xoff = ctx->in.crop.left;
	...
	cmd[reg >> 2] |= FIELD_PREP(RGA3_WIN_ENABLE, 1)
		      ...
		      |  FIELD_PREP(RGA3_WIN_YUV10B_COMPACT, in->yuv10b_compact);
	...
	// 	      | FIELD_PREP(RGA3_HEIGHT, src_h);
		...
		/* set size */
	reg = RGA3_WIN0_ACT_OFF - RGA3_FIRST_CMD_REG;
	cmd[reg >> 2] = FIELD_PREP(RGA3_WIDTH, act_xoff)
		      | FIELD_PREP(RGA3_HEIGHT, act_yoff);
	reg = RGA3_WIN0_ACT_SIZE - RGA3_FIRST_CMD_REG;
	cmd[reg >> 2] = FIELD_PREP(RGA3_WIDTH, act_w)
		      | FIELD_PREP(RGA3_HEIGHT, act_h);
	reg = RGA3_WIN0_SRC_SIZE - RGA3_FIRST_CMD_REG;
...
static struct rga3_fmt rga3_formats[] = {
	...
	/*
	 * NV15: 10-bit YUV420SP compact, input only.
	 * Use RGA3_COLOR_FMT_YUV420_10B + RGA3_WIN_YUV10B_COMPACT.
	 * Output must be NV12 (RGA3 downconverts 10→8 bit in hardware).
	 */
	{
		.fourcc         = V4L2_PIX_FMT_NV15,
		.hw_format      = RGA3_COLOR_FMT_YUV420_10B,
		.yuv10b_compact = 1,
		.input_only     = 1,
	},
};
...
const struct rga_hw rga3_hw = {
	...
	.stride_alignment = 64,
	.features = RGA_FEATURE_SELECTION,
	//.features = 0,
