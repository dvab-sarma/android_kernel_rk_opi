...
/* bit 10: compact packing for 10-bit YUV raster input (TRM §5.5 WIN0_CTRL[10]) */
#define RGA3_WIN_YUV10B_COMPACT BIT(10)
...
struct rga3_fmt {
	...
	bool rbuv_swap;
	bool yc_swap;
	bool yuv10b_compact; /* set RGA3_WIN_YUV10B_COMPACT for compact 10-bit YUV input */
	bool input_only;     /* format not available on the CAPTURE (output) queue */
};
