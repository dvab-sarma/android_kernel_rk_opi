...
static int vidioc_try_fmt(struct file *file, void *priv, struct v4l2_format *f)
{
	...
	u32 req_bpl[VIDEO_MAX_PLANES];
	unsigned int i;

	/*
	 * Remember any explicit per-plane stride the caller requested before
	 * it gets overwritten below. A caller copying from a decoder or into
	 * a gralloc buffer whose real row stride doesn't follow
	 * hw->stride_alignment can request a wider stride to match the real
	 * DMA-BUF layout instead of getting a silently wrong default.
	 */
	for (i = 0; i < pix_fmt->num_planes && i < VIDEO_MAX_PLANES; i++)
		req_bpl[i] = pix_fmt->plane_fmt[i].bytesperline;
	...
	/*
	 * Honor an explicit wider stride per plane. sizeimage is stride times
	 * row-count for every plane/format this driver supports, so scaling
	 * it by the same ratio stays exact without duplicating per-format
	 * chroma-height math here.
	 */
	for (i = 0; i < pix_fmt->num_planes && i < VIDEO_MAX_PLANES; i++) {
		u32 old_bpl = pix_fmt->plane_fmt[i].bytesperline;

		if (req_bpl[i] >= pix_fmt->width && req_bpl[i] != old_bpl && old_bpl > 0) {
			pix_fmt->plane_fmt[i].sizeimage =
				(u32)div_u64((u64)pix_fmt->plane_fmt[i].sizeimage *
					     req_bpl[i], old_bpl);
			pix_fmt->plane_fmt[i].bytesperline = req_bpl[i];
		}
	}
